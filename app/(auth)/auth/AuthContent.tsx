...
export default function AuthPage() {
  ...
  async function onSignup(e: React.FormEvent) {
    ...
    const result = await signupUser({
      ...
      email,
      password,
      phone: showPhoneForm ? phone : undefined,
      role,
                  ...
                  {!showPhoneForm && (
                    <>
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-[0.55rem] py-3 bg-white border-[1.5px] border-offwhite-dark rounded-lg text-[0.85rem] font-medium text-ink transition-all touch-manipulation active:scale-[0.98] mb-6"
                      >
                        <PhoneIcon />
                        Continue with Phone
                      </button>
                  ...
                  {showPhoneForm && (
                    <>
                      <button
                        ...
                        className="text-[0.82rem] text-orange hover:underline cursor-pointer mb-4 inline-block"
                      >
                        ← Back to email
                      </button>
...
/* ─── Phone Form (Unified for Signup & Signin) ─── */
function PhoneForm({
  mode,
  phone,
  onPhoneChange,
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    const result = await verifyPhoneOTP(phone, otp, options);
    setLocalLoading(false);
    if (!result.success) {
